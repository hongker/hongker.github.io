---
title: Go数据结构与算法(12)-时间轮计时器
date: 2022-07-19 20:33:22
tags: algorithm
---
...
<!--more-->
![timewheel.png](timewheel.png)

## 实现
基于延时队列来实现时间轮定时器，底层算法是堆排序。关于延时队列请参考：[Go数据结构与算法(11)-延时队列](https://hongker.github.io/2022/07/17/algorithm-delayqueue/)
...
func New(tick time.Duration, wheelSize int64) *TimeWheel {
	...
	delayQueue := queue.NewDelayQueue(int(wheelSize))
	return newTimeWheel(tickMs, wheelSize,UnixMilli(time.Now()), delayQueue)
}

func newTimeWheel(tickMs int64, wheelSize int64, startMs int64, delayQueue *queue.DelayQueue) *TimeWheel {
	// 初始化任务桶
